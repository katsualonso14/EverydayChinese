...
class HealthViewController: UITableViewController, AVAudioPlayerDelegate, AVSpeechSynthesizerDelegate {
    ...
    let favoritesLocalKey = "favoriteContacts_health"
    ...
    func CustomCellTapButtonCall(cell: UITableViewCell, pushTime: TimeInterval) {
        ...
        let contact = sentenceView.healthSentenceArray[indexPathTapped.section].names[indexPathTapped.row]
        let hasFavorited = contact.hasFavorited
        
        sentenceView.healthSentenceArray[indexPathTapped.section].names[indexPathTapped.row].hasFavorited = !hasFavorited
        saveFavorites()

        content.title = contact.name
        content.body = contact.name
        content.sound = UNNotificationSound.default
        content.userInfo = ["page": "health"]
    ...
    func CustomCellTapButtonCall2(cell: UITableViewCell, pushTime: TimeInterval) {
        ...
        sentenceView.healthSentenceArray[indexPathTapped.section].names[indexPathTapped.row].hasFavorited2 = !hasFavorited
        saveFavorites()
    ...
    func CustomCellTapButtonCall3(cell: UITableViewCell, pushTime: TimeInterval) {
        ...
        sentenceView.healthSentenceArray[indexPathTapped.section].names[indexPathTapped.row].hasFavorited3 = !hasFavorited
        saveFavorites()
    ...
    func CustomCellTapButtonCall4(cell: UITableViewCell, pushTime: TimeInterval) {
        ...
        sentenceView.healthSentenceArray[indexPathTapped.section].names[indexPathTapped.row].hasFavorited4 = !hasFavorited
        saveFavorites()
    ...
    func saveFavorites() {
        if let encoded = try? JSONEncoder().encode(sentenceView.healthSentenceArray[0].names) {
    ...
    func loadFavorites() {
        if let savedData = UserDefaults.standard.data(forKey: favoritesLocalKey),
           let decoded = try? JSONDecoder().decode([Contact].self, from: savedData) {
            sentenceView.healthSentenceArray = [ExpandableNames(isExpanded: true, names: decoded)]
        }
    }

    // Cellの数
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return sentenceView.healthSentenceArray[0].names.count
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let contact = sentenceView.healthSentenceArray[0].names[indexPath.row]
        cell.setCell(sentence: sentenceView.healthSentence[indexPath.row], pronunciation: sentenceView.healthPronunciation[indexPath.row], japanese: sentenceView.healthEnglish[indexPath.row])
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let utterance = AVSpeechUtterance.init(string: sentenceView.healthSentence[indexPath.row])
    ...
    func addRemindList(tappedRow: Int) {
        let data = ["sentence": sentenceView.healthSentence[tappedRow]]
        ...
        if var savedRemindData = UserDefaults.standard.stringArray(forKey: "remind") {
            savedRemindData.append(sentenceView.healthSentence[tappedRow])
            UserDefaults.standard.set(savedRemindData, forKey: "remind")
        } else {
            UserDefaults.standard.set([sentenceView.healthSentence[tappedRow]], forKey: "remind")
        }
    }

    func deleteRemindList(tappedRow: Int) {
        let dataToDelete = ["sentence": sentenceView.healthSentence[tappedRow]]
        NotificationCenter.default.post(name: Notification.Name("deleteRemind"), object: nil, userInfo: dataToDelete)

        if var savedRemindData = UserDefaults.standard.stringArray(forKey: "remind") {
            savedRemindData.removeAll { $0 == sentenceView.healthSentence[tappedRow] }
            UserDefaults.standard.set(savedRemindData, forKey: "remind")
        }
    }
}
