...
class PersonalPronounsViewController: UITableViewController,AVAudioPlayerDelegate, AVSpeechSynthesizerDelegate {
    ...
    let favoritesLocalKey = "favoriteContacts_pronouns_personal"
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.personalpronounsVC = self
        
        
        let contact = sentenceView.personalPronounsSentenceArray[0].names[indexPath.row]
        //cellの文字指定
        cell.setCell(sentence: sentenceView.personalPronounsSentence[indexPath.row], pronunciation: sentenceView.personalPronounsPronunciation[indexPath.row], japanese: sentenceView.personalPronounsEnglish[indexPath.row
        ])
...
//        print("tap")
    ...
    func CustomCellTapButtonCall(cell: UITableViewCell, pushTime: TimeInterval) {
        ...
        let contact = sentenceView.personalPronounsSentenceArray[indexPathTapped.section].names[indexPathTapped.row]
        print(contact)
        let hasFavorited = contact.hasFavorited
        
        sentenceView.personalPronounsSentenceArray[indexPathTapped.section].names[indexPathTapped.row].hasFavorited = !hasFavorited
        saveFavorites() // ハートボタンの色の状態を保存
        //タップしてときの値をpushメッセージに記載
        content.title = contact.name
        content.body = contact.name
        content.sound = UNNotificationSound.default
        content.userInfo = ["page": "personal"]
        //通知設定
    ...
    func CustomCellTapButtonCall2(cell: UITableViewCell, pushTime: TimeInterval) {
        ...
        sentenceView.personalPronounsSentenceArray[indexPathTapped.section].names[indexPathTapped.row].hasFavorited2 = !hasFavorited
        saveFavorites() // ハートボタンの色の状態を保存
        //タップしてときの値をpushメッセージに記載
        content.title = contact.name
        content.body = contact.name
    ...
    func CustomCellTapButtonCall3(cell: UITableViewCell, pushTime: TimeInterval) {
        ...
        sentenceView.personalPronounsSentenceArray[indexPathTapped.section].names[indexPathTapped.row].hasFavorited3 = !hasFavorited
        saveFavorites() // ハートボタンの色の状態を保存
        //タップしてときの値をpushメッセージに記載
        content.title = contact.name
        content.body = contact.name
    ...
    func CustomCellTapButtonCall4(cell: UITableViewCell, pushTime: TimeInterval) {
        ...
        sentenceView.personalPronounsSentenceArray[indexPathTapped.section].names[indexPathTapped.row].hasFavorited4 = !hasFavorited
        saveFavorites() // ハートボタンの色の状態を保存
        //タップしてときの値をpushメッセージに記載
        content.title = contact.name
        content.body = contact.name
    ...
    // ハートボタンの状態をローカルに保存
    func saveFavorites() {
        if let encoded = try? JSONEncoder().encode(sentenceView.personalPronounsSentenceArray[0].names) {
    ...
    func loadFavorites() {
        if let savedData = UserDefaults.standard.data(forKey: favoritesLocalKey),
           let decoded = try? JSONDecoder().decode([Contact].self, from: savedData) {
            sentenceView.personalPronounsSentenceArray = [ExpandableNames(isExpanded: true, names: decoded)]
        }
    }
    //プッシュ通知登録
    ...
    func addRemindList(tappedRow: Int) {
        // 別VCへの値渡し
        let data = ["sentence": sentenceView.personalPronounsSentence[tappedRow]]
        ...
        if var savedRemindData = UserDefaults.standard.stringArray(forKey: "remind") {
            savedRemindData.append(sentenceView.personalPronounsSentence[tappedRow])
            UserDefaults.standard.set(savedRemindData, forKey: "remind")
        } else {
            UserDefaults.standard.set([sentenceView.personalPronounsSentence[tappedRow]], forKey: "remind")
        }
        
    }
    //RemindListからの削除
    func deleteRemindList(tappedRow: Int) {
        let dataToDelete = ["sentence": sentenceView.personalPronounsSentence[tappedRow]]
        NotificationCenter.default.post(name: Notification.Name("deleteRemind"), object: nil, userInfo: dataToDelete)
        
        //ローカルからの削除
        if var savedRemindData = UserDefaults.standard.stringArray(forKey: "remind") {
            savedRemindData.removeAll { $0 == sentenceView.personalPronounsSentence[tappedRow] }
            UserDefaults.standard.set(savedRemindData, forKey: "remind")
        } else {
