...
class BusinessViewController: UITableViewController, AVAudioPlayerDelegate, AVSpeechSynthesizerDelegate {
    ...
    let favoritesLocalKey = "favoriteContacts_business"
    ...
    func CustomCellTapButtonCall(cell: UITableViewCell, pushTime: TimeInterval) {
        ...
        sentenceView.businessSentenceArray[indexPathTapped.section].names[indexPathTapped.row].hasFavorited = !hasFavorited
        saveFavorites()
    ...
    func saveFavorites() {
        if let encoded = try? JSONEncoder().encode(sentenceView.businessSentenceArray[0].names) {
    ...
    func loadFavorites() {
        if let savedData = UserDefaults.standard.data(forKey: favoritesLocalKey),
           let decoded = try? JSONDecoder().decode([Contact].self, from: savedData) {
            sentenceView.businessSentenceArray = [ExpandableNames(isExpanded: true, names: decoded)]
        }
    }

    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return sentenceView.businessSentenceArray[0].names.count
    }

    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as! CustomTableViewCell
        cell.businessVC = self

        let contact = sentenceView.businessSentenceArray[0].names[indexPath.row]
        cell.setCell(sentence: sentenceView.businessSentence[indexPath.row], pronunciation: sentenceView.businessPronunciation[indexPath.row], japanese: sentenceView.businessEnglish[indexPath.row])
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let utterance = AVSpeechUtterance.init(string: sentenceView.businessSentence[indexPath.row])
    ...
    func deleteRemindList(tappedRow: Int) {
        let dataToDelete = ["sentence": sentenceView.businessSentence[tappedRow]]
        NotificationCenter.default.post(name: Notification.Name("deleteRemind"), object: nil, userInfo: dataToDelete)

        if var savedRemindData = UserDefaults.standard.stringArray(forKey: "remind") {
            savedRemindData.removeAll { $0 == sentenceView.businessSentence[tappedRow] }
            UserDefaults.standard.set(savedRemindData, forKey: "remind")
        }
    }
}
